...
# model.cpu()
model.embedding_manager.load('logs/modern_embeddings.pt')
model = model.to(device)



def main(prompt='', content_dir='', style_dir='', ddim_steps=50, strength=0.5, model=None, seed=50):
    ddim_eta = 0.0
    n_iter = 1
    C = 4
    f = 8
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                #output.save(os.path.join(outpath, content_name + '-' + prompt + f'-{grid_count:04}.jpg'))
                Image.fromarray(grid.astype(np.uint8)).save(os.path.join(outpath, '1.png'))
                grid_count += 1
...
if __name__ == "__main__":
    ...
    main(prompt='hair',
         content_dir='pexels-aleksandar-pasaric-2041556.jpg',
         style_dir='modern.jpg',
         ddim_steps=50,
         strength=0.9,
         seed=50,
         model=model)